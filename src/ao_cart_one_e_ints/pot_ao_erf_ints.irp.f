...
double precision function NAI_pol_mult_erf_ao_cart(i_ao, j_ao, mu_in, C_center)
  ...
  num_A         = ao_cart_nucl(i_ao)
  power_A(1:3)  = ao_cart_power(i_ao,1:3)
  A_center(1:3) = nucl_coord(num_A,1:3)
  num_B         = ao_cart_nucl(j_ao)
  power_B(1:3)  = ao_cart_power(j_ao,1:3)
  ...
  do i = 1, ao_cart_prim_num(i_ao)
    ...
    do j = 1, ao_cart_prim_num(j_ao)
      beta = ao_cart_expo_ordered_transp(j,j_ao)
...
double precision function NAI_pol_mult_erf_ao_cart_with1s(i_ao, j_ao, beta, B_center, mu_in, C_center)
  ...
  if(beta .lt. 1d-10) then
    NAI_pol_mult_erf_ao_cart_with1s = NAI_pol_mult_erf_ao(i_ao, j_ao, mu_in, C_center)
    return
  endif

  power_A1(1:3) = ao_cart_power(i_ao,1:3)
  power_A2(1:3) = ao_cart_power(j_ao,1:3)

  A1_center(1:3) = nucl_coord(ao_cart_nucl(i_ao),1:3)
  A2_center(1:3) = nucl_coord(ao_cart_nucl(j_ao),1:3)

  n_pt_in = n_pt_max_integrals

  NAI_pol_mult_erf_ao_cart_with1s = 0.d0
  do i = 1, ao_cart_prim_num(i_ao)
    alpha1 = ao_cart_expo_ordered_transp           (i,i_ao)
    coef1  = ao_cart_coef_normalized_ordered_transp(i,i_ao)

    do j = 1, ao_cart_prim_num(j_ao)
      alpha2 = ao_cart_expo_ordered_transp(j,j_ao)
      coef12 = coef1 * ao_cart_coef_normalized_ordered_transp(j,j_ao)
      ...
      NAI_pol_mult_erf_ao_cart_with1s += integral * coef12
    enddo
  enddo

end function NAI_pol_mult_erf_ao_cart_with1s
