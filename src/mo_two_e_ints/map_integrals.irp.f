...
double precision function get_two_e_integral(i,j,k,l,map)
  ...
  if (iand(ii, -mo_integrals_cache_size) == 0) then
    ...
    if  (do_mo_cholesky) then
      ...
      call gpu_set_stream(gpu_blas, stream)
    ...
    if (mo_integrals_cache_min > 1) then
      ...
      if (do_mo_cholesky) then
        ...
        if (mo_cholesky_double) then
          ...
          call gpu_dgemv(blas_handle,'T', cholesky_mo_num, mo_integrals_cache_min-1, 1.d0, &
    ...
    if (mo_integrals_cache_max < mo_num) then
      ...
      if (do_mo_cholesky) then
        ...
        if (mo_cholesky_double) then
          ...
          call gpu_dgemv(blas_handle, 'T', cholesky_mo_num, mo_num-mo_integrals_cache_max, 1.d0, &
          ...
          call gpu_sgemv(blas_handle, 'T', cholesky_mo_num, mo_num-mo_integrals_cache_max, 1., &
...
!          enddo
    ...
    if (do_mo_cholesky) then
      ...
      if (mo_cholesky_double) then
          ...
          call gpu_dgemv(blas_handle, 'T', cholesky_mo_num, sze, 1.d0, &
               cholesky_mo_transp_d%f(1,1,k), cholesky_mo_num, &
               cholesky_mo_transp_d%f(1,j,l), 1, 0.d0, &
               out_val_d%f(1), 1)
          call gpu_download(out_val_d,out_val)
          ...
          call gpu_sgemv(blas_handle, 'T', cholesky_mo_num, sze, 1., &
               cholesky_mo_transp_sp_d%f(1,1,k), cholesky_mo_num, &
               cholesky_mo_transp_sp_d%f(1,j,l), 1, 0., &
               out_val_sp_d%f(1), 1)
          call gpu_download(out_val_sp_d,out_val_sp)
